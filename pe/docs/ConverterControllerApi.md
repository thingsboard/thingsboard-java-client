...
```
void deleteConverter(DeleteConverterArgs args) // Delete converter (deleteConverter)
Converter getConverterById(GetConverterByIdArgs args) // Get Converter (getConverterById)
PageDataConverter getConverters(GetConvertersArgs args) // Get Converters (getConverters)
List<Converter> getConvertersByIds(GetConvertersByIdsArgs args) // Get Converters By Ids (getConvertersByIds)
com.fasterxml.jackson.databind.JsonNode getLatestConverterDebugInput(GetLatestConverterDebugInputArgs args) // Get latest debug input event (getLatestConverterDebugInput)
Converter saveConverter(SaveConverterArgs args) // Create Or Update Converter (saveConverter)
com.fasterxml.jackson.databind.JsonNode testDownLinkConverter(TestDownLinkConverterArgs args) // Test converter function (testDownLinkConverter)
com.fasterxml.jackson.databind.JsonNode testUpLinkConverter(TestUpLinkConverterArgs args) // Test converter function (testUpLinkConverter)
com.fasterxml.jackson.databind.JsonNode unwrapRawPayload(UnwrapRawPayloadArgs args) // Transform input raw payload to the dedicated converter data (unwrapRawPayload)
```
...
```java
void deleteConverter(DeleteConverterArgs args)
// build the request (required fields shown; add optional fields from the table below as needed):
DeleteConverterArgs.builder()
        .converterId(String)
        .build()
```

### `DeleteConverterArgs` builder fields

| Field | Type | Required | Description | Notes |
|-------|------|----------|-------------|-------|
| `converterId` | `String` | **yes** | A string value representing the converter id. For example, '784f394c-42b6-435a-983c-b7beff2784f9' | |
...
```java
Converter getConverterById(GetConverterByIdArgs args)
// build the request (required fields shown; add optional fields from the table below as needed):
GetConverterByIdArgs.builder()
        .converterId(String)
        .build()
```

### `GetConverterByIdArgs` builder fields
...
```java
List<Converter> getConvertersByIds(GetConvertersByIdsArgs args)
// build the request (required fields shown; add optional fields from the table below as needed):
GetConvertersByIdsArgs.builder()
        .converterIds(List<String>)
        .build()
```

### `GetConvertersByIdsArgs` builder fields

| Field | Type | Required | Description | Notes |
|-------|------|----------|-------------|-------|
| `converterIds` | `List<String>` | **yes** | A list of converter ids, separated by comma ',' | |
...
```java
com.fasterxml.jackson.databind.JsonNode getLatestConverterDebugInput(GetLatestConverterDebugInputArgs args)
// build the request (required fields shown; add optional fields from the table below as needed):
GetLatestConverterDebugInputArgs.builder()
        .converterId(String)
        .build()
```

### `GetLatestConverterDebugInputArgs` builder fields

| Field | Type | Required | Description | Notes |
|-------|------|----------|-------------|-------|
| `converterId` | `String` | **yes** | A string value representing the converter id. For example, '784f394c-42b6-435a-983c-b7beff2784f9' | |
| `converterType` | `String` | no | A string value representing the converter type. One of the following: UPLINK, DOWNLINK | |
| `integrationType` | `String` | no | A string value representing the integration type. One of the following: APACHE_PULSAR, AWS_IOT, AWS_KINESIS, AWS_SQS, AZURE_EVENT_HUB, AZURE_IOT_HUB, AZURE_SERVICE_BUS, CHIRPSTACK, COAP, CUSTOM, HTTP, IBM_WATSON_IOT, KAFKA, LORIOT, MQTT, OCEANCONNECT, OPC_UA, PUB_SUB, RABBITMQ,  SIGFOX,  TCP,  THINGPARK,  TMOBILE_IOT_CDP,  TPE,  TTI,  TTN,  TUYA,  UDP | |
| `integrationName` | `String` | no | A string value representing the integration name. For example, 'My New Integration' | |
| `converterVersion` | `Integer` | no | Converter version. | |
...
```java
Converter saveConverter(SaveConverterArgs args)
// build the request (required fields shown; add optional fields from the table below as needed):
SaveConverterArgs.builder()
        .converter(Converter)
        .build()
```

### `SaveConverterArgs` builder fields

| Field | Type | Required | Description | Notes |
|-------|------|----------|-------------|-------|
| `converter` | `Converter` | **yes** | A JSON value representing the converter. | |
...
```java
com.fasterxml.jackson.databind.JsonNode testDownLinkConverter(TestDownLinkConverterArgs args)
// build the request (required fields shown; add optional fields from the table below as needed):
TestDownLinkConverterArgs.builder()
        .body(Object)
        .build()
```

### `TestDownLinkConverterArgs` builder fields

| Field | Type | Required | Description | Notes |
|-------|------|----------|-------------|-------|
| `body` | `Object` | **yes** | A JSON value representing the input to the converter function. | |
...
```java
com.fasterxml.jackson.databind.JsonNode testUpLinkConverter(TestUpLinkConverterArgs args)
// build the request (required fields shown; add optional fields from the table below as needed):
TestUpLinkConverterArgs.builder()
        .body(Object)
        .build()
```

### `TestUpLinkConverterArgs` builder fields
...
```java
com.fasterxml.jackson.databind.JsonNode unwrapRawPayload(UnwrapRawPayloadArgs args)
// build the request (required fields shown; add optional fields from the table below as needed):
UnwrapRawPayloadArgs.builder()
        .integrationType(IntegrationType)
        .body(Object)
        .build()
```

### `UnwrapRawPayloadArgs` builder fields

| Field | Type | Required | Description | Notes |
|-------|------|----------|-------------|-------|
| `integrationType` | `IntegrationType` | **yes** | A string value representing the integration type. One of the following: APACHE_PULSAR, AWS_IOT, AWS_KINESIS, AWS_SQS, AZURE_EVENT_HUB, AZURE_IOT_HUB, AZURE_SERVICE_BUS, CHIRPSTACK, COAP, CUSTOM, HTTP, IBM_WATSON_IOT, KAFKA, LORIOT, MQTT, OCEANCONNECT, OPC_UA, PUB_SUB, RABBITMQ,  SIGFOX,  TCP,  THINGPARK,  TMOBILE_IOT_CDP,  TPE,  TTI,  TTN,  TUYA,  UDP | enum: `OCEANCONNECT`, `SIGFOX`, `THINGPARK`, `TPE`, `CHIRPSTACK`, `PARTICLE`, `TMOBILE_IOT_CDP`, `HTTP`, `MQTT`, `PUB_SUB`, `AWS_IOT`, `AWS_SQS`, `AWS_KINESIS`, `TTN`, `TTI`, `AZURE_EVENT_HUB`, `OPC_UA`, `CUSTOM`, `UDP`, `TCP`, `KAFKA`, `AZURE_IOT_HUB`, `APACHE_PULSAR`, `RABBITMQ`, `LORIOT`, `COAP`, `TUYA`, `AZURE_SERVICE_BUS`, `KPN` |
| `body` | `Object` | **yes** | A JSON value representing the input message. | |
