/**
 * Copyright © 2026-2026 ThingsBoard, Inc.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
package org.thingsboard.client.model;

import java.net.URLEncoder;
import java.nio.charset.StandardCharsets;
import java.util.StringJoiner;
import java.util.Objects;
import java.util.Map;
import java.util.HashMap;
import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.annotation.JsonCreator;
import com.fasterxml.jackson.annotation.JsonTypeName;
import com.fasterxml.jackson.annotation.JsonValue;
import java.util.Arrays;
import com.fasterxml.jackson.annotation.JsonPropertyOrder;


import org.thingsboard.client.ApiClient;
/**
 * DebugSettings
 */
@JsonPropertyOrder({
  DebugSettings.JSON_PROPERTY_FAILURES_ENABLED,
  DebugSettings.JSON_PROPERTY_ALL_ENABLED,
  DebugSettings.JSON_PROPERTY_ALL_ENABLED_UNTIL
})
@javax.annotation.Generated(value = "org.openapitools.codegen.languages.JavaClientCodegen", date = "2026-02-24T16:17:50.774971+02:00[Europe/Kiev]", comments = "Generator version: 7.20.0")
public class DebugSettings {
  public static final String JSON_PROPERTY_FAILURES_ENABLED = "failuresEnabled";
  @javax.annotation.Nullable
  private Boolean failuresEnabled;

  public static final String JSON_PROPERTY_ALL_ENABLED = "allEnabled";
  @javax.annotation.Nullable
  private Boolean allEnabled;

  public static final String JSON_PROPERTY_ALL_ENABLED_UNTIL = "allEnabledUntil";
  @javax.annotation.Nullable
  private Long allEnabledUntil;

  public DebugSettings() { 
  }

  public DebugSettings failuresEnabled(@javax.annotation.Nullable Boolean failuresEnabled) {
    this.failuresEnabled = failuresEnabled;
    return this;
  }

  /**
   * Debug failures. 
   * @return failuresEnabled
   */
  @javax.annotation.Nullable
  @JsonProperty(value = JSON_PROPERTY_FAILURES_ENABLED, required = false)
  @JsonInclude(value = JsonInclude.Include.USE_DEFAULTS)
  public Boolean getFailuresEnabled() {
    return failuresEnabled;
  }


  @JsonProperty(value = JSON_PROPERTY_FAILURES_ENABLED, required = false)
  @JsonInclude(value = JsonInclude.Include.USE_DEFAULTS)
  public void setFailuresEnabled(@javax.annotation.Nullable Boolean failuresEnabled) {
    this.failuresEnabled = failuresEnabled;
  }


  public DebugSettings allEnabled(@javax.annotation.Nullable Boolean allEnabled) {
    this.allEnabled = allEnabled;
    return this;
  }

  /**
   * Debug All. Used as a trigger for updating debugAllUntil.
   * @return allEnabled
   */
  @javax.annotation.Nullable
  @JsonProperty(value = JSON_PROPERTY_ALL_ENABLED, required = false)
  @JsonInclude(value = JsonInclude.Include.USE_DEFAULTS)
  public Boolean getAllEnabled() {
    return allEnabled;
  }


  @JsonProperty(value = JSON_PROPERTY_ALL_ENABLED, required = false)
  @JsonInclude(value = JsonInclude.Include.USE_DEFAULTS)
  public void setAllEnabled(@javax.annotation.Nullable Boolean allEnabled) {
    this.allEnabled = allEnabled;
  }


  public DebugSettings allEnabledUntil(@javax.annotation.Nullable Long allEnabledUntil) {
    this.allEnabledUntil = allEnabledUntil;
    return this;
  }

  /**
   * Timestamp of the end time for the processing debug events.
   * @return allEnabledUntil
   */
  @javax.annotation.Nullable
  @JsonProperty(value = JSON_PROPERTY_ALL_ENABLED_UNTIL, required = false)
  @JsonInclude(value = JsonInclude.Include.USE_DEFAULTS)
  public Long getAllEnabledUntil() {
    return allEnabledUntil;
  }


  @JsonProperty(value = JSON_PROPERTY_ALL_ENABLED_UNTIL, required = false)
  @JsonInclude(value = JsonInclude.Include.USE_DEFAULTS)
  public void setAllEnabledUntil(@javax.annotation.Nullable Long allEnabledUntil) {
    this.allEnabledUntil = allEnabledUntil;
  }


  /**
   * Return true if this DebugSettings object is equal to o.
   */
  @Override
  public boolean equals(Object o) {
    if (this == o) {
      return true;
    }
    if (o == null || getClass() != o.getClass()) {
      return false;
    }
    DebugSettings debugSettings = (DebugSettings) o;
    return Objects.equals(this.failuresEnabled, debugSettings.failuresEnabled) &&
        Objects.equals(this.allEnabled, debugSettings.allEnabled) &&
        Objects.equals(this.allEnabledUntil, debugSettings.allEnabledUntil);
  }

  @Override
  public int hashCode() {
    return Objects.hash(failuresEnabled, allEnabled, allEnabledUntil);
  }

  @Override
  public String toString() {
    StringBuilder sb = new StringBuilder();
    sb.append("class DebugSettings {\n");
    sb.append("    failuresEnabled: ").append(toIndentedString(failuresEnabled)).append("\n");
    sb.append("    allEnabled: ").append(toIndentedString(allEnabled)).append("\n");
    sb.append("    allEnabledUntil: ").append(toIndentedString(allEnabledUntil)).append("\n");
    sb.append("}");
    return sb.toString();
  }

  /**
   * Convert the given object to string with each line indented by 4 spaces
   * (except the first line).
   */
  private String toIndentedString(Object o) {
    if (o == null) {
      return "null";
    }
    return o.toString().replace("\n", "\n    ");
  }

  /**
   * Convert the instance into URL query string.
   *
   * @return URL query string
   */
  public String toUrlQueryString() {
    return toUrlQueryString(null);
  }

  /**
   * Convert the instance into URL query string.
   *
   * @param prefix prefix of the query string
   * @return URL query string
   */
  public String toUrlQueryString(String prefix) {
    String suffix = "";
    String containerSuffix = "";
    String containerPrefix = "";
    if (prefix == null) {
      // style=form, explode=true, e.g. /pet?name=cat&type=manx
      prefix = "";
    } else {
      // deepObject style e.g. /pet?id[name]=cat&id[type]=manx
      prefix = prefix + "[";
      suffix = "]";
      containerSuffix = "]";
      containerPrefix = "[";
    }

    StringJoiner joiner = new StringJoiner("&");

    // add `failuresEnabled` to the URL query string
    if (getFailuresEnabled() != null) {
      joiner.add(String.format(java.util.Locale.ROOT, "%sfailuresEnabled%s=%s", prefix, suffix, ApiClient.urlEncode(ApiClient.valueToString(getFailuresEnabled()))));
    }

    // add `allEnabled` to the URL query string
    if (getAllEnabled() != null) {
      joiner.add(String.format(java.util.Locale.ROOT, "%sallEnabled%s=%s", prefix, suffix, ApiClient.urlEncode(ApiClient.valueToString(getAllEnabled()))));
    }

    // add `allEnabledUntil` to the URL query string
    if (getAllEnabledUntil() != null) {
      joiner.add(String.format(java.util.Locale.ROOT, "%sallEnabledUntil%s=%s", prefix, suffix, ApiClient.urlEncode(ApiClient.valueToString(getAllEnabledUntil()))));
    }

    return joiner.toString();
  }
}

