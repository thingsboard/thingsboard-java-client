...
| Field | Type | Required | Description | Notes |
|-------|------|----------|-------------|-------|
| `notificationType` | `NotificationType` | **yes** |  | enum: `GENERAL`, `ALARM`, `DEVICE_ACTIVITY`, `ENTITY_ACTION`, `ALARM_COMMENT`, `RULE_ENGINE_COMPONENT_LIFECYCLE_EVENT`, `ALARM_ASSIGNMENT`, `NEW_PLATFORM_VERSION`, `ENTITIES_LIMIT`, `ENTITIES_LIMIT_INCREASE_REQUEST`, `API_USAGE_LIMIT`, `RULE_NODE`, `RATE_LIMITS`, `EDGE_CONNECTION`, `EDGE_COMMUNICATION_FAILURE`, `TASK_PROCESSING_FAILURE`, `RESOURCES_SHORTAGE` |
