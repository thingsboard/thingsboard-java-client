...
```
Long countAlarmsByQuery(CountAlarmsByQueryArgs args) // Count Alarms by Query (countAlarmsByQuery)
Long countEntitiesByQuery(CountEntitiesByQueryArgs args) // Count Entities by Query
PageDataAlarmData findAlarmDataByQuery(FindAlarmDataByQueryArgs args) // Find Alarms by Query
AvailableEntityKeys findAvailableEntityKeysByQuery(FindAvailableEntityKeysByQueryArgs args) // Find Available Entity Keys by Query (deprecated)
AvailableEntityKeysV2 findAvailableEntityKeysByQueryV2(FindAvailableEntityKeysByQueryV2Args args) // Find Available Entity Keys By Query
PageDataEntityData findEntityDataByQuery(FindEntityDataByQueryArgs args) // Find Entity Data by Query
EdqsState getEdqsState() // getEdqsState
void processSystemEdqsRequest(ProcessSystemEdqsRequestArgs args) // processSystemEdqsRequest
```
...
```java
Long countAlarmsByQuery(CountAlarmsByQueryArgs args)
// build the request (required fields shown; add optional fields from the table below as needed):
CountAlarmsByQueryArgs.builder()
        .alarmCountQuery(AlarmCountQuery)
        .build()
```

### `CountAlarmsByQueryArgs` builder fields

| Field | Type | Required | Description | Notes |
|-------|------|----------|-------------|-------|
| `alarmCountQuery` | `AlarmCountQuery` | **yes** |  | |
...
```java
Long countEntitiesByQuery(CountEntitiesByQueryArgs args)
// build the request (required fields shown; add optional fields from the table below as needed):
CountEntitiesByQueryArgs.builder()
        .entityCountQuery(EntityCountQuery)
        .build()
```

### `CountEntitiesByQueryArgs` builder fields

| Field | Type | Required | Description | Notes |
|-------|------|----------|-------------|-------|
| `entityCountQuery` | `EntityCountQuery` | **yes** |  | |
...
```java
PageDataAlarmData findAlarmDataByQuery(FindAlarmDataByQueryArgs args)
// build the request (required fields shown; add optional fields from the table below as needed):
FindAlarmDataByQueryArgs.builder()
        .alarmDataQuery(AlarmDataQuery)
        .build()
```

### `FindAlarmDataByQueryArgs` builder fields

| Field | Type | Required | Description | Notes |
|-------|------|----------|-------------|-------|
| `alarmDataQuery` | `AlarmDataQuery` | **yes** |  | |
...
```java
AvailableEntityKeys findAvailableEntityKeysByQuery(FindAvailableEntityKeysByQueryArgs args)
// build the request (required fields shown; add optional fields from the table below as needed):
FindAvailableEntityKeysByQueryArgs.builder()
        .timeseries(Boolean)
        .attributes(Boolean)
...
### `FindAvailableEntityKeysByQueryArgs` builder fields

| Field | Type | Required | Description | Notes |
|-------|------|----------|-------------|-------|
| `timeseries` | `Boolean` | **yes** | When true, includes unique time series key names in the response. When false, the 'timeseries' list will be empty. | |
| `attributes` | `Boolean` | **yes** | When true, includes unique attribute key names in the response. When false, the 'attribute' list will be empty. Use 'scope' parameter to filter by attribute scope. | |
| `entityDataQuery` | `EntityDataQuery` | **yes** |  | |
| `scope` | `String` | no | Filters attribute keys by scope. Only applies when 'attributes' is true. If not specified, returns attribute keys from all scopes. | enum: `SERVER_SCOPE`, `SHARED_SCOPE`, `CLIENT_SCOPE` |
...
```java
AvailableEntityKeysV2 findAvailableEntityKeysByQueryV2(FindAvailableEntityKeysByQueryV2Args args)
// build the request (required fields shown; add optional fields from the table below as needed):
FindAvailableEntityKeysByQueryV2Args.builder()
        .entityDataQuery(EntityDataQuery)
        .build()
```

### `FindAvailableEntityKeysByQueryV2Args` builder fields

| Field | Type | Required | Description | Notes |
|-------|------|----------|-------------|-------|
| `entityDataQuery` | `EntityDataQuery` | **yes** |  | |
| `includeTimeseries` | `Boolean` | no | When true, includes unique time series keys in the response. When false, the 'timeseries' field is omitted. At least one of 'includeTimeseries' or 'includeAttributes' must be true. | default: `true` |
| `includeAttributes` | `Boolean` | no | When true, includes unique attribute keys in the response. When false, the 'attributes' field is omitted. At least one of 'includeTimeseries' or 'includeAttributes' must be true. | default: `true` |
| `scopes` | `String` | no | Filters attribute keys by scope. Only applies when 'includeAttributes' is true. When not specified, scopes are auto-determined: all three scopes (server, client, shared) for device entities, server scope only for other entity types. | enum: `SERVER_SCOPE`, `SHARED_SCOPE`, `CLIENT_SCOPE` |
| `includeSamples` | `Boolean` | no | When true, each key entry includes a 'sample' object with the most recent value and timestamp. When false, only key names are returned (sample is omitted from JSON). | default: `false` |
...
```java
PageDataEntityData findEntityDataByQuery(FindEntityDataByQueryArgs args)
// build the request (required fields shown; add optional fields from the table below as needed):
FindEntityDataByQueryArgs.builder()
        .entityDataQuery(EntityDataQuery)
        .build()
```

### `FindEntityDataByQueryArgs` builder fields
...
```java
void processSystemEdqsRequest(ProcessSystemEdqsRequestArgs args)
// build the request (required fields shown; add optional fields from the table below as needed):
ProcessSystemEdqsRequestArgs.builder()
        .toCoreEdqsRequest(ToCoreEdqsRequest)
        .build()
```

### `ProcessSystemEdqsRequestArgs` builder fields

| Field | Type | Required | Description | Notes |
|-------|------|----------|-------------|-------|
| `toCoreEdqsRequest` | `ToCoreEdqsRequest` | **yes** |  | |
